...
class DeleteTeam(commands.Cog):
    ...
    async def delete_team(self, interaction, team_name: str):
        ...
        if not db.team_exists(team_name):
            await interaction.response.send_message("Team name doesn't exist", ephemeral = True)
            return
        ...
        await interaction.response.send_message(f"Succesfully deleted team {team_name}", ephemeral = True)
        pass
