...
class DropSubmissionModal(ui.Modal):
    ...
    async def on_submit(self, modal_interaction: discord.Interaction):
        print(f"DropSubmissionModal on_submit started for user {modal_interaction.user.id}")
        try:
            ...
            print(f"Drop submission payload prepared successfully")

            try:
                async with aiohttp.ClientSession() as session:
                    print(f"Sending request to DROP_SERVER_URL: {DROP_SERVER_URL}")
                    async with session.post(DROP_SERVER_URL, json=payload) as response:
                        print(f"Response received from server with status: {response.status}")
                ...
                print(f"Drop Submission - API Error: {str(e)}")
                print(traceback.format_exc())
                await modal_interaction.followup.send(f"An error occurred during submission: {str(e)}", ephemeral=True)
        except Exception as outer_e:
            print(f"Drop Submission - Outer Exception: {str(outer_e)}")
            print(traceback.format_exc())
            if not modal_interaction.response.is_done():
                await modal_interaction.response.send_message("Oops! Something went wrong with the drop submission form.", ephemeral=True)
            else:
                try:
                    await modal_interaction.followup.send("Oops! Something went wrong with the drop submission form.", ephemeral=True)
                except Exception as followup_e:
                    print(f"Failed to send followup error message: {str(followup_e)}")

    async def on_error(self, error: Exception, interaction: discord.Interaction) -> None:
        print(f"Error in DropSubmissionModal on_error: {error}")
        print(traceback.format_exc())
        try:
            ...
            print(f"Failed to send error message in on_error: {str(e)}")
...
class KCSubmissionModal(ui.Modal):
    ...
    async def on_submit(self, modal_interaction: discord.Interaction):
        print(f"KCSubmissionModal on_submit started for user {modal_interaction.user.id}")
        try:
            await modal_interaction.response.defer(ephemeral=True, thinking=True)
            print("KC modal interaction deferred successfully")

            try:
                kc_value = int(self.questionKillCount.value)
            except ValueError:
                print(f"Invalid KC value provided: {self.questionKillCount.value}")
            ...
            print(f"KC submission payload prepared successfully")

            try:
                async with aiohttp.ClientSession() as session:
                    print(f"Sending request to KC_SERVER_URL: {KC_SERVER_URL}")
                ...
                print(f"KC Submission - API Error: {str(e)}")
                print(traceback.format_exc())
                await modal_interaction.followup.send(f"An error occurred during submission: {str(e)}", ephemeral=True)
        except Exception as outer_e:
            print(f"KC Submission - Outer Exception: {str(outer_e)}")
            print(traceback.format_exc())
            if not modal_interaction.response.is_done():
                await modal_interaction.response.send_message("Oops! Something went wrong with the KC submission form.", ephemeral=True)
            else:
                try:
                    await modal_interaction.followup.send("Oops! Something went wrong with the KC submission form.", ephemeral=True)
                except Exception as followup_e:
                    print(f"Failed to send followup error message: {str(followup_e)}")

    async def on_error(self, error: Exception, interaction: discord.Interaction) -> None:
        print(f"Error in KCSubmissionModal on_error: {error}")
        print(traceback.format_exc())
        try:
            if not interaction.response.is_done():
...
class SubmissionTypeView(ui.View):
    ...
    async def drop_button_callback(self, button_object: ui.Button, interaction_from_button_click: discord.Interaction):
        ...
        try:
            ...
            print("Drop modal sent successfully")
    ...
    async def kc_button_callback(self, button_object: ui.Button, interaction_from_button_click: discord.Interaction):
        ...
        try:
            ...
            print("KC modal sent successfully")
            await self._disable_buttons_and_edit_original()
        except Exception as e:
